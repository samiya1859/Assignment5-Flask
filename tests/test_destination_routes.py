...
def test_add_destination_as_admin(client):
    ...
    register_response = client.post(
        "/register", json=admin_register_data, content_type="application/json"
    )
    ...
    # Get admin's token from the login response
    admin_token = login_response.get_json()["auth_token"]
...
def test_get_all_destinations(client, logged_in_user):
    ...
    response = client.get(
        "/destinations", headers={"Authorization": logged_in_user["auth_token"]}
    )
...
def test_get_destination_by_id(client, logged_in_user):
    ...
    response = client.get(
        f"/destinations/{first_destination_id}",
        headers={"Authorization": logged_in_user["auth_token"]},
    )
...
def test_update_destination_as_admin(client):
    ...
    # First, get a destination ID
    destinations_response = client.get(
        "/destinations", headers={"Authorization": admin_token}
    )
    destinations = destinations_response.get_json()
    first_destination_id = destinations[0]["id"]
...
def test_delete_destination_as_admin(client):
    ...
    response = client.delete(
        f"/destinations/{first_destination_id}", headers={"Authorization": admin_token}
    )
...
def test_get_nonexistent_destination(client, logged_in_user):
    ...
    response = client.get(
        "/destinations/nonexistent_id",
        headers={"Authorization": logged_in_user["auth_token"]},
    )

    assert response.status_code == 404
    assert "Destination" in response.get_json()["message"]
