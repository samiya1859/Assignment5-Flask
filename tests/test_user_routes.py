...
def test_logout(client):
    ...
    login_response = client.post('/login', json={"email": user_data["email"], "password": user_data["password"]})
...
def test_get_all_users(client):
    ...
    assert any(user["email"] == admin_data["email"] for user in users_data["users"]), "Admin user not found"
...
def test_delete_user(client):
    ...
    assert any(user["email"] == user_data["email"] for user in users["users"]), "User to delete not found"
    ...
    response = client.get('/users', headers=headers)
    users = response.get_json()
    assert not any(user["email"] == user_data["email"] for user in users), "User not deleted"
